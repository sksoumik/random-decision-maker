...
function App() {
  ...
  const drawSpinner = () => {
    ...
    const innerSize = isMobile ? 12 : 15;
    ctx.beginPath();
  ...
  const drawWatchHand = () => {
    ...
    const centerX = size / 2;
    const centerY = size / 2;
    const handLength = Math.min(centerX, centerY) - 25;
    
    ctx.save();
    ctx.translate(centerX, centerY);
    ctx.rotate((handRotation * Math.PI) / 180);
    
    const handWidth = isMobile ? 4 : 6;
    const shadowWidth = isMobile ? 6 : 8;
    ...
    // Draw hand tip
    const tipSize = isMobile ? 6 : 8;
    ctx.beginPath();
    ctx.arc(0, -handLength + 5, tipSize, 0, 2 * Math.PI);
  ...
  useEffect(() => {
    drawSpinner();
    drawWatchHand();
  }, [options, handRotation, windowSize]);
      ...
      <div className="relative z-10 p-3 sm:p-4 md:p-6 lg:p-8">
        ...
        <div className="text-center mb-4 sm:mb-6 md:mb-8">
          ...
          <p className="text-sm sm:text-lg md:text-xl lg:text-2xl text-white/80 font-light px-4">
            {isMobile ? "Spin for your choice!" : "Watch the hand point to your destiny!"}
          </p>
        </div>
        ...
        <div className="text-center mt-8 text-white/60">
          ...
          {!isMobile && (
            <p className="text-xs sm:text-sm mt-2">🕐 Enhanced with precision timing and smooth animations</p>
          )}
        </div>
      </div>
    </div>
